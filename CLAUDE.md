...
- AI chat: UI in `src/chat/`, endpoint in `netlify/functions/chat.mts` (TanStack AI + Gemini; server env vars listed in `.env.example`). Page sections live in `SECTIONS` (`src/chat/tools.ts`) — the TOC and the chat's navigation tool both derive from it, so add new sections there. Conversations are logged to Upstash Redis under `concerti:chat:log:*` (TTL-bounded, keyed by the client thread id) and served back by `chat-history.mts` (`/api/chat/history`) so a visitor can reopen/continue their own past chats (the widget keeps the ids in localStorage); shared key layout in `src/chat/chatlog.ts`.
