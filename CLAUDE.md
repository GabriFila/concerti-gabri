...
- AI chat: UI in `src/chat/`, endpoint in `netlify/functions/chat.mts` (TanStack AI + Gemini; server env vars listed in `.env.example`). Page sections live in `SECTIONS` (`src/chat/tools.ts`) — the TOC and the chat's navigation tool both derive from it, so add new sections there. Conversations are logged to Upstash Redis under `concerti:chat:log:*` (TTL-bounded, keyed by the client thread id) and served back by `chat-history.mts` (`/api/chat/history`): reading is public (deliberate — no accounts), continuing requires the thread's write key, which lives only in the owner browser's localStorage. Shared key layout in `src/chat/chatlog.ts`.
